...
import dev.ProJakob.mods.ModInstances;
...
public class EntityPotion extends EntityThrowable
{
    ...
    public void onUpdate() {
    	if(ModInstances.getModArrowTrail().isEnabled()) {
    		for (int k = 0; k < 4; ++k)
            {
                this.worldObj.spawnParticle(EnumParticleTypes.REDSTONE, this.posX + this.motionX * (double)k / 4.0D, this.posY + this.motionY * (double)k / 4.0D, this.posZ + this.motionZ * (double)k / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ, new int[0]);
            }
    	}
    	super.onUpdate();
    }
